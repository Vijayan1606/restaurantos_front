...
import {
  LayoutDashboard, UtensilsCrossed, ClipboardList, BookOpen, Boxes,
  Truck, Users2, Receipt, ScanLine, Sparkles, LogOut, ChefHat,
  Calendar, History, X,
} from 'lucide-react';
...
const links = [
  ...
  { to: '/recipes', label: 'Recipes', icon: ChefHat, roles: ['manager', 'chef'] },
  { to: '/shifts', label: 'Staff Shifts', icon: Calendar, roles: ['manager'] },
  { to: '/activity-logs', label: 'Activity Logs', icon: History, roles: ['manager'] },
  { to: '/inventory', label: 'Inventory', icon: Boxes, roles: ['manager', 'store_manager'] },
  { to: '/purchase-orders', label: 'Purchase Orders', icon: Truck, roles: ['manager', 'store_manager'] },
  { to: '/suppliers', label: 'Suppliers', icon: Users2, roles: ['manager', 'store_manager'] },
  { to: '/expenses', label: 'Expenses', icon: Receipt, roles: ['manager', 'cashier'] },
  { to: '/invoices', label: 'AI Invoices', icon: ScanLine, roles: ['manager', 'cashier', 'store_manager'] },
  { to: '/ai-insights', label: 'AI Insights', icon: Sparkles, roles: ['manager', 'store_manager', 'chef'] },
];
    ...
    <aside className="w-64 bg-white/90 backdrop-blur-xl border-r border-orange-100/60 flex flex-col h-screen sticky top-0 z-20">
      <div className="p-5 flex items-center justify-between border-b border-orange-100/60">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-lg font-extrabold gradient-text leading-tight">RestaurantOS</h1>
            <p className="text-[11px] text-gray-400 -mt-0.5">AI Restaurant Platform</p>
          </div>
        </div>
        <button
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {visible.map((l) => {
          ...
          const Icon = l.icon;
          return (
            <NavLink key={l.to} to={l.to} className="relative block" onClick={closeMobileSidebar}>
              {isActive && (
                <motion.div
              ...
              <span
                className={`relative z-10 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors duration-150 ${
                  isActive ? 'text-white' : 'text-gray-600 hover:bg-brand-50 hover:text-brand-700'
                }`}
              >
      ...
      <div className="p-3 border-t border-orange-100/60">
        <div className="flex items-center gap-2.5 mb-3 px-1">
          ...
          <div className="min-w-0">
            ...
            <span className={`badge ${ROLE_COLORS[user?.role] || 'bg-gray-100 text-gray-600'}`}>{user?.role}</span>
          </div>
        </div>
        <button onClick={() => { logout(); closeMobileSidebar && closeMobileSidebar(); }} className="btn-secondary w-full">
