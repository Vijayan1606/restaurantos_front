...
export default function Recipes() {
  ...
  const selectedRecipe = selectedItem
    ? recipes.find((r) => r.menu_item_id === selectedItem.id)
    : null;
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Base Cost: ₹{selectedItem.cost_price || 0} · Sale
                          Price: ₹{selectedItem.price} · Prep:{" "}
                          {selectedItem.avg_prep_time_minutes} min
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="card space-y-4">
                          ...
                          <table className="data-table w-full">
                            ...
                            <tbody>
                              {currentRecipeIngredients.map((ri) => {
                                ...
                                const ingCost = matchedIng
                                  ? parseFloat(matchedIng.unit_cost)
                                  : 0;
                                const lineCost =
                                  ingCost * parseFloat(ri.quantity);
                                return (
                                  <tr key={ri.id}>
                                    <td className="font-medium text-gray-700">
                                      {ri.ing_ingredient_id_name ||
                                        matchedIng?.name ||
                                        "—"}
                                    </td>
                                    <td>
                                      {ri.quantity}{" "}
                                      {ri.ing_ingredient_id_unit ||
                                        matchedIng?.unit ||
                                        ""}
                                    </td>
                        ...
                        <form
                          ...
                          className="border-t border-gray-100 pt-4 flex flex-col sm:flex-row gap-3"
                        >
                            ...
                            >
                              ...
                              {ingredients.map((ing) => (
                                <option key={ing.id} value={ing.id}>
                                  {ing.name} ({ing.unit}) — ₹
                                  {parseFloat(ing.unit_cost).toFixed(2)}
                                </option>
                              ))}
                            </select>
                          </div>
                    ...
                    <div className="card flex flex-col items-center text-center py-16 space-y-4">
                      ...
                      <div>
                        <h4 className="font-bold text-gray-800 text-lg">
                          No Recipe Initialized
                        </h4>
                        <p className="text-gray-400 text-sm max-w-sm mt-1">
                          This menu item does not have a recipe configuration
                          yet. Create one to compile instructions and track
                ...
                <div className="card flex flex-col items-center justify-center text-center py-32 space-y-4 h-full border border-dashed border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400 text-xs mt-1 max-w-xs">
                      Choose a menu item from the catalog on the left to review
                      or edit its recipe formula and step-by-step instructions.
                    </p>
                  </div>
                </div>
              )}
